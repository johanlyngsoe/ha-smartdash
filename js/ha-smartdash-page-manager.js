...
window.BeastPageManager = (() => {
  const STANDARD = [
    ...
    ["waste", "Kalender", "calendar"], ["school", "Skole", "school"], ["shopping", "Indkøb", "cart"], ["robots", "Robotter", "robot"], ["printer", "3D Printer", "printer"]
  ].map(([id, label, icon]) => ({ id, label, icon, standard: true }));
  const ICONS = ["grid", "home", "cloud", "camera", "shield", "music", "bolt", "thermometer", "car", "droplet", "calendar", "school", "cart", "robot", "printer", "sparkles"];
